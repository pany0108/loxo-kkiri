  ...
  return (
    <div className="flex flex-col min-h-dvh bg-white dark:bg-gray-950 font-['Pretendard']">
      ...
      <div ref={scrollContainerRef} className="flex-1 px-6 pt-[calc(76px+env(safe-area-inset-top))] pb-8 overflow-y-auto w-full">
        {/* 헤더 섹션 */}
        <PageHeader className="mb-10" icon={<Lock className="text-[#478BCC] w-6 h-6" />}>
          <h2 className="text-2xl font-black text-gray-900 dark:text-white leading-[1.3] tracking-tight">
            {isResetMode ? (
              <>
                잊으신 비밀번호를 <br />
                <span className="text-[#478BCC]">새로 설정할게요</span>
              </>
            ) : (
              <>
                보안을 위해 <br />
                <span className="text-[#478BCC]">비밀번호를 변경할게요</span>
              </>
            )}
          </h2>
                ...
                foundEmail && (
                  <div className="animate-in fade-in duration-300 space-y-4">
                    <div className="text-center bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl">
                      ...
                      <p className="text-lg font-bold text-[#478BCC] mt-1">{foundEmail.masked}</p>
                    </div>
                    <FormInput
                      label="이메일 주소 확인"
                      icon={<Mail size={20} />}
