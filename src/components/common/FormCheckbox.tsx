      ...
      <div className="relative">
        ...
        <div
          className={`w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center ${
            checked ? 'bg-blue-600 border-blue-600' : 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 group-hover:border-blue-400'
          }`}
        >
          {checked && <Check size={14} className="text-white" strokeWidth={4} />}
        </div>
      </div>
      <span className={`text-sm font-bold transition-colors ${checked ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400'}`}>{label}</span>
    </label>
  );
};
