...
const ReportHeader: React.FC<ReportHeaderProps> = ({ title, location, status, confirmedSlot, scheduleId, onNavigate, isRetry }) => {
  ...
  return (
    <header className="mb-8">
      ...
      <h3 className="text-lg font-bold text-sub dark:text-gray-400 mb-2">{title}</h3>
      {location && (
      ...
      {status === 'CONFIRMED' ? (
        <>
          <PageTitle>
            약속이 <span className="text-primary dark:text-blue-400">확정</span>되었습니다!
          </PageTitle>
          <div className="mt-4 bg-primary/10 dark:bg-blue-900/50 p-4 rounded-2xl border border-primary/20 dark:border-blue-800">
            <p className="text-center text-lg font-black text-primary dark:text-blue-300">{confirmedDateDisplay}</p>
            <p className="text-center text-2xl font-black text-primary dark:text-blue-300">{confirmedSlot?.date.includes(':') ? '' : confirmedSlot?.time}</p>
          </div>
        ...
        <>
          <PageTitle>
            {isRetry ? (
              <>
                재요청된 약속의 <br />
                <span className="text-primary dark:text-blue-400">시간</span>을 확정해주세요!
              </>
            ) : (
              <>
                가장 <span className="text-primary dark:text-blue-400">적절한 시간</span>을<br />
                확정해주세요!
              </>
            )}
          </PageTitle>
          <p className="mt-2 text-sub dark:text-gray-500 text-sm font-medium flex items-center gap-1.5">
