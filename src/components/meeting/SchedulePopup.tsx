    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-5 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-[32px] p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-black text-gray-900 dark:text-white">{dayjs(date).format('M월 D일의 일정')}</h3>
          <button onClick={onClose} className="p-2 -mr-2 text-gray-300 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-bold rounded-[18px] text-sm">
            닫기
          </button>
          <button
