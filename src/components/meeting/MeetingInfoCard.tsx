...
const MeetingInfoCard: React.FC<MeetingInfoCardProps> = ({ title, description, location }) => {
  return (
    <div className="bg-gray-50 dark:bg-gray-800 rounded-[28px] p-6 mb-10 border border-gray-100 dark:border-gray-700/50 shadow-sm">
      ...
      <h3 className="text-[19px] font-black text-gray-900 dark:text-white mb-3">{title}</h3>

      <div className="space-y-3">
        <div className="flex items-start gap-2.5">
          <AlignLeft size={16} className="text-gray-400 dark:text-gray-500 mt-0.5 shrink-0" />
          <p className="text-[14px] font-medium text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{description || '설명 없음'}</p>
        </div>
        {location && (
          <div className="flex items-start gap-2.5">
