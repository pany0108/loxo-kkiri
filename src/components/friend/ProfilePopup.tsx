      ...
      <div className="relative w-full max-w-xs bg-white dark:bg-gray-800 rounded-[32px] p-6 text-center shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="w-24 h-24 rounded-full mx-auto mb-4 overflow-hidden border-4 border-white dark:border-gray-700 shadow-lg">
          {friend.photoURL ? (
            ...
            <div className="w-full h-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 flex items-center justify-center text-4xl font-bold">{friend.name[0]}</div>
          )}
        </div>
        <h3 className="text-xl font-black text-gray-900 dark:text-white">{friend.name}</h3>
        <p className="text-sm text-gray-400 dark:text-gray-500 font-medium mt-1 mb-4">{friend.email}</p>
