...
const RecurrenceOptions = ({ startDate, value, onChange }: RecurrenceOptionsProps) => {
  ...
  return (
    <div className="space-y-3">
      <label className="block text-[13px] font-black text-gray-400 dark:text-gray-500 ml-1">반복 설정</label>
        ...
        className={`flex items-center justify-between h-[60px] px-5 rounded-[20px] border-2 transition-all cursor-pointer
          ${
            value.frequency !== 'none'
              ...
              : 'bg-gray-50 dark:bg-gray-800/50 border-transparent text-gray-800 dark:text-gray-200'
          }
        `}
      >
        ...
        <ChevronDown
          size={18}
          className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} ${value.frequency !== 'none' ? 'text-blue-400' : 'text-gray-300 dark:text-gray-600'}`}
        />
      </div>
            ...
            {['none', 'daily', 'weekly', 'monthly', 'yearly'].map((freq) => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-[12px] font-bold whitespace-nowrap px-3 transition-all
                  ${value.frequency === freq ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'}
                `} // prettier-ignore
              >
          ...
          {value.frequency !== 'none' && (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-1.5 border border-gray-200 dark:border-gray-600 focus-within:border-blue-500 focus-within:bg-white dark:focus-within:bg-gray-700 transition-all">
                  <input
                    type="text"
                    inputMode="numeric"
                    ...
                    {WEEKDAYS.map((day, idx) => (
                      <button
                        ...
                        className={`w-9 h-9 rounded-full text-[12px] font-bold transition-all
                          ${value.daysOfWeek.includes(idx) ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600'}
                        `}
                      >
                        {day}
                      </button>
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer">
                    ...
                    <input
                      ...
                      className="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-md px-2 py-1 text-[12px] outline-none focus:border-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-700/50 disabled:text-gray-400 transition-colors"
                    />
                  </label>
